...
export const signup = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const lawyerID = generateUniqueLawyerID(); // Moved inside

      if (!fullName || !password || !confirmPassword || !email || !contact) {
      ...
      console.error("Error in signup controller:", err.message, err.stack);
